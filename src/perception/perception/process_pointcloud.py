...
class RealSensePCSubscriber(Node):
    ...
    def pointcloud_callback(self, msg: PointCloud2):
        ...
        # ------------------------
        points_in_range = [p for p in points if p[2] < self.max_distance]

       
        # Apply other filtering relative to plane
        
        #EVERYTHING BELOW MAX_DIST + ABOVE THE TABLE
        filtered_points = np.array([p for p in points_in_range if p[1] > 
                           ((self.a * p[0]) + (self.b * p[1]) + (self.c * p[2]) + self.d)
                           ])
        

        # Compute position of the cube via remaining points
        cube_x = sum([p[0] for p in filtered_points]) / len(filtered_points)
        cube_y = sum([p[1] for p in filtered_points]) / len(filtered_points)
        cube_z = sum([p[2] for p in filtered_points]) / len(filtered_points)
